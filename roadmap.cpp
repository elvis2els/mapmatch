...
{
    Box box = range_box(p, radious);    //获得查询矩阵范围
    vector<int> ret;
...
//    }
    ...
    sort(cand.begin(), cand.end(), [](pair<int, double> const & a, pair<int, double> const & b)
    {
        return a.second < b.second;
    });
    for (auto & p : cand)
...
//        if ( finishSet.count(top.cross))
        if ( finishSet.find(top.cross) != finishSet.end())
        {
            ...
            reverse(cross_index.begin(), cross_index.end());
//            b::reverse(cross_index);
            return cross_index;
        }
...
Path RoadMap::shortest_path(ProjectPoint const& start, ProjectPoint const& end) const
{
    ...
    else if ( start.type == ProjectPoint::OnRoad && end.type == ProjectPoint::OnRoad )  //OnRoad OnRoad
    {
        ...
        else  //different road
        {
            ...
            Path sPath;
            vector<int> start_possible_cross = possible_start_nearby_cross(start, rs);      //返回下一个可能经过的路口index号
...
//                if ( endG.count(v) )
            ...
            if ( !cross_index.empty() )
            {
                ...
                sPath.entities.push_front(part_of_road_s[b::find<b::return_begin_found>(start_possible_cross, cross_index.front()).size()]);    //size() = findit - begin
